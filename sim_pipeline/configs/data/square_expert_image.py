...
class PushingDataConfig(BaseDataConfig):
    ...
    env_type: list[str] | None = field(default_factory=lambda: [EnvType.ROBOMIMIC.value])
    dataset_type: list[str] | None  = field(default_factory=lambda: [DatasetType.PH.value])
    action_type: str = ActionType.RELATIVE.value
    reward_type: list[str] | None = field(default_factory=lambda: [RewardType.SPARSE.value])
