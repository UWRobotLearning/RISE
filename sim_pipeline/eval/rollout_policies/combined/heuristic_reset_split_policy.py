...
class HeuristicResetSplitPolicy(CompositeRolloutPolicy):
    def __init__(self, policies: list[RolloutPolicy], n_envs=1, reset_steps=400, init_run_steps=35):
    ...
    def switch_policy(self, obs, act, env) -> bool:
        ...
        if self.steps < self.init_run_steps:
            ...
            self.env_to_policy_idx = {env: 2 for env in range(self.n_envs)}
            self.n_obs_steps = self.n_obs_steps_dict[2]
            self.action_chunking = self.action_chunking_dict[2]
            if self.curr_policy != 2:
                self.curr_policy = 2
                switched = True
        else:
